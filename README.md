...
Note: In the project there is an example of comparing to codes. "myCode.cpp" calculates sum of subarray by prefix sum technique, "answer.cpp" calculates the same by brute force. input is size of array(maximum 100 number) in one line and the array(maximum number is billion) in another line and number of queries in one line and in each line two idices in array refers to the left index and right index you should print the sum of the segment between.
