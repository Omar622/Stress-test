...
{
    std::string name = "";
    while (!path.empty() and (path.back() != '\\' or path.back() != '/'))
        name.push_back(path.back()), path.pop_back();
    reverse(name.begin(), name.end());
    return name;
}

// convert ..\..\foo.cpp to ..\..\foo.exe
...
void FileControl::build()
{
    ...
    runCommand(command, "can not build " + path_to_name(cpp_file_path));
}
...
void FileControl::run()
{
    ...
    runCommand(command, "can not excute " + path_to_name(exe_file_path));
}
